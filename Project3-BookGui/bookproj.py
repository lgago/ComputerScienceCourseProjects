...
def friends(name, nfriends=2):
    ...
    friendsList = friendsList[0:nfriends]
    return friendsList
...
def recommend(name, nfriends=2):
    Friends = friends(name, nfriends)
    recommendations = []

    #Remove books read by current person from friends lists.
    friendsList = []
    for items in Friends:
        friendsList.append(ratings[items])
    
    for items in friendsList:
    ...
    #pull the 3 and 5's for each friend out of ratings. 
    for items in friendsList: 
        recommendations.append(getIndexPositions(items, 3))   
        recommendations.append(getIndexPositions(items, 5))
...
def report():
    ...
    for people in sortedKeys:
        ...
        megaString += recommend(people)
        megaString += "\n" + "\n"
    return megaString
        # print(people, str(": "), twoFriends ,recommend(people))
